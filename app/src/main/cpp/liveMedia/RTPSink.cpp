...
#define DEBUG_TIMESTAMPS TRUE

FILE *logfile1 = fopen("/storage/emulated/0/AudioRecorderMp3/recorder/log.txt", "w+");//不能定义多处
////////// RTPSink //////////
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
  ...
  fprintf(logfile1, "fTimestampBase: 0x%08x, tv: %lu.%06ld\n\t=> RTP timestamp: 0x%08x\n",
...
::noteIncomingRR(u_int32_t SSRC, struct sockaddr_in const& lastFromAddress,
  ...
  if (stats == NULL) {
    ...
    add(SSRC, stats);
#ifdef DEBUG_RR
    fprintf(logfile1, "Adding new entry for SSRC %x in RTPTransmissionStatsDB\n", SSRC);
#endif
  }
...
::noteIncomingRR(struct sockaddr_in const& lastFromAddress,
...
#ifdef DEBUG_RR
  fprintf(logfile1, "RTCP RR data (received at %lu.%06ld): lossStats 0x%08x, lastPacketNumReceived 0x%08x, jitter 0x%08x, lastSRTime 0x%08x, diffSR_RRTime 0x%08x\n",
  ...
  fprintf(logfile1, "=> round-trip delay: 0x%04x (== %f seconds)\n", rtd, rtd/65536.0);
#endif
