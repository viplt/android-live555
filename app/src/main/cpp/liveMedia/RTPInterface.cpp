...
FILE *logfile = fopen("/storage/emulated/0/AudioRecorderMp3/recorder/log.txt", "w+");
...
RTPInterface::RTPInterface(Medium* owner, Groupsock* gs)
  ...
  increaseSendBufferTo(envir(), fGS->socketNum(), 100*1024);//-----lt 默认50*1024
}
...
#ifdef DEBUG_SEND
  fprintf(logfile, "sendRTPorRTCPPacketOverTCP: %d bytes over channel %d (socket %d)\n",
...
#ifdef DEBUG_SEND
    fprintf(logfile, "sendRTPorRTCPPacketOverTCP: completed\n"); fflush(stderr);
#endif
...
#ifdef DEBUG_SEND
  fprintf(logfile, "sendRTPorRTCPPacketOverTCP: failed! (errno %d)\n", envir().getErrno()); fflush(stderr);
#endif
  return False;
}
...
#ifdef DEBUG_SEND
      fprintf(logfile, "sendDataOverTCP: resending %d-byte send (blocking)\n", numBytesRemainingToSend); fflush(stderr);
#endif
...
#ifdef DEBUG_SEND
	fprintf(logfile, "sendDataOverTCP: blocking send() failed (delivering %d bytes out of %d); closing socket %d\n", sendResult, numBytesRemainingToSend, socketNum); fflush(stderr);
#endif
...
void SocketDescriptor::registerRTPInterface(unsigned char streamChannelId,
  ...
  fprintf(logfile, "SocketDescriptor(socket %d)::registerRTPInterface(channel %d): isFirstRegistration %d\n", fOurSocketNum, streamChannelId, isFirstRegistration);
#endif
...
#if defined(DEBUG_SEND)||defined(DEBUG_RECEIVE)
  fprintf(logfile, "SocketDescriptor(socket %d)::deregisterRTPInterface(channel %d)\n", fOurSocketNum, streamChannelId);
#endif
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
...
#ifdef DEBUG_RECEIVE
      fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): readSocket(1 byte) returned %d (error)\n", fOurSocketNum, result);
#endif
  ...
  switch (fTCPReadingState) {
    case AWAITING_DOLLAR: {
      if (c == '$') {
#ifdef DEBUG_RECEIVE
	fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): Saw '$'\n", fOurSocketNum);
#endif
...
#ifdef DEBUG_RECEIVE
	fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): Saw nonexistent stream channel id: 0x%02x\n", fOurSocketNum, c);
#endif
...
#ifdef DEBUG_RECEIVE
	  fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): reading %d bytes on channel %d\n", fOurSocketNum, rtpInterface->fNextTCPReadSize, rtpInterface->fNextTCPReadStreamChannelId);
#endif
	  fTCPReadingState = AWAITING_PACKET_DATA;
	  rtpInterface->fReadHandlerProc(rtpInterface->fOwner, mask);
	} else {
#ifdef DEBUG_RECEIVE
	  fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): No handler proc for \"rtpInterface\" for channel %d; need to skip %d remaining bytes\n", fOurSocketNum, fStreamChannelId, rtpInterface->fNextTCPReadSize);
#endif
	  int result = readSocket(fEnv, fOurSocketNum, &c, 1, fromAddress);
...
#ifdef DEBUG_RECEIVE
      else fprintf(logfile, "SocketDescriptor(socket %d)::tcpReadHandler(): No \"rtpInterface\" for channel %d\n", fOurSocketNum, fStreamChannelId);
#endif
    }
  }
