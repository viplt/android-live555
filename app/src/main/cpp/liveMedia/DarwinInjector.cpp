...
float DarwinInjector::duration() const {
...
//    float dur = duration();
//    if (dur == 0.0) {
//      rangeLine = strDup("a=range:npt=0-10\r\n");
//    } else if (dur > 0.0) {
//      char buf[100];
...
//            "%s";// a=auxSDPLine: line;
    ...
    unsigned sdpLen = strlen(sdpFmt)
                      ...
                      + strlen(rangeLine)// a=range: line  不能为空保证必须有
                      + strlen(sessionName)
                      + strlen(sessionInfo)
                      ...
                      + fSubstreamSDPSizes;
//                      + strlen(auxSDPLine);// a=auxSDPLine: line  不能为空保证必须有;
    ...
    // Finally, make sure that the output TCP buffer is a reasonable size: -----lt
...
//  char const* auxSDPLine = fRTPSink->auxSDPLine();
  char const* auxSDPLine = fRTPSink->getAuxSDPLine();
  ...
  //拼装fmt
//  char* newauxSDPLine;
//  char const* const auxFmt = "a=audiofmt:%d/16/%d/%d\r\n";
//  unsigned auxFmtSize = strlen(auxFmt)
//                           + 20
//                           + 1;
//  newauxSDPLine = new char[auxFmtSize];
////
//  if(rtpTimestampFrequency == 16000){
//    sprintf(newauxSDPLine,auxFmt,rtpPayloadType,rtpTimestampFrequency,1);
//  }else{
//    sprintf(newauxSDPLine,auxFmt,rtpPayloadType,rtpTimestampFrequency,2);
//  }
//  unsigned auxSDPLineSize = strlen(newauxSDPLine);
...
//          "a=audiofmt:14/16/44100/2\r\na=control:trackID=%u\r\n";
//          "a=audiofmt:14/16/16000/2\r\na=control:trackID=%u\r\n";
//          "a=audiofmt:14/16/24000/2\r\na=control:trackID=%u\r\n";//在x-qt-text-inf中传audiofmt过来
          "a=control:trackID=%u\r\n";
...
//          newauxSDPLine, // optional extra SDP line
