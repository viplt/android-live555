...
//                encodeThread.flushAndRelease();
     ...
     */
    public DataEncodeThread(File file, int bufferSize, LivePush lPush) throws FileNotFoundException {
    ...
    private int processData() {
        if (mTasks.size() > 0) {
            ...
            if (encodedSize > 0) {
                try {
                    if(livePush != null && livePush.isConn){
                        livePush.pushBuffer(mMp3Buffer,null);
                    }
//                    mFileOutputStream.write(mMp3Buffer, 0, encodedSize);//异步写数据
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
